...
mutable struct states
    ...
    # profile_indices::Array{Int, 1}             # Array of plasmid profile index
...
mutable struct rates
    ...
    #infection_rates::Array{Array{Float64, 1}, 1} # for the old infection equation
...
function initialize_strains(propensities, params)
    ...
    # profile_indices = [findfirst(==(p), all_profiles) for p in plasmid_profiles] #/ e.g. if all_profiles[1] = [0,0] = plasmid_profiles[3], then profile_indice[3] = 1
    ...
    for i in donors
        ...
        # for (j, plasmid_profile) in enumerate(plasmid_profiles)
        #     #/ Check if donor can infect recipient
        #     if any(donor_plasmid_profile .> plasmid_profile)
        #         push!(recipients_of_donor, j)
        #     end
        # end
    ...
    #/ Create subpouplation indices in propensitie tensors for donors and recipients
...
function get_background_rates(
    ...
    #plasmid_resistances = Array{Float64}(undef, 0)
    #plasmid_costs = Array{Float64}(undef, 0)
	...
	for (strain_id, plasmid_profile) in zip(states.subpopulations, states.plasmid_profiles)
        ...
        # strain_abundance = sum(states.abundances[states.subpopulations .== strain_id]) #/ get the focal substrain's strain abundance
        # η = (isempty(_plasmids)) ?
        # growth_rate[strain_id] * (1- strain_abundance / carrying_capacity[strain_id]) :
        # growth_rate[strain_id] * (1- strain_abundance / carrying_capacity[strain_id]) * reduce((x, y) -> x * (1.0 - y), plasmid_cost[_plasmids], init=1.0) # taking the sequential product of (1-cost) from each plasmid

        # Option 3: Including intra-specific competition; considering K = sum of K_i 
        # K = sum(carrying_capacity) #/ get community carrying capacity
        # N = sum(states.abundances) #/ get the community abundance
        # η = (isempty(_plasmids)) ?
        # growth_rate[strain_id] * (1- N / K) :
        # growth_rate[strain_id] * (1- N / K) * reduce((x, y) -> x * (1.0 - y), plasmid_cost[_plasmids], init=1.0) # taking the sequential product of (1-cost) from each plasmid
...
## Functions that imposes time-dependent impact
#/ linearly constant/increasing/decreasing with time, with perturbation parameter (treated as constant/final/initial strength of perturbation) and slope (if strength is not constant) defined
function incr_linear(time, perturb, slope)
    if slope == 0 # constant across time
        return perturb
    elseif slope > 0 # increasing with time up to maximum perturbation
        return 0 + min(slope * time, perturb)
    else # slope < 0, i.e. decreasing with time down to minimum perturbation (0)
        return max(perturb + (slope * time), 0)  
    end    
end

#/ fluctuating with time (0 at t = 0), with perturbation parameter (treated as the fold of μ_i as peak-to-peak amplitude), and fluctuation period defined
function custom_sine(time, perturb, period)
    y = (perturb / 2) * (sin(2π / period * time) + 1)         
    return y
end
# e.g. custom_sine(time, 1.5, 84)  
...
#/ single pulse + exponentially decreasing with time, with perturbation parameter (treated as the fold of μ_i as initial strength of pulse), pulse time t_p (hour) and the decay constant k defined
function pulse_expd_single(time, perturb, t_p, k)
     if time < t_p
        return 0.0
     else
        return perturb * exp(-k * (time - t_p))    
     end
end

#/ periodic pulses (with a time lag in the beginning) + exponentially decreasing with time, with perturbation parameter (treated as the fold of μ_i as initial strength of pulse), pulse period t_p (hour) and the decay constant k defined
# (the bigger the perturb, the higher the impulse peak)
function pulse_expd_periodic(lag::Any, time::Any, perturb::Any, t_p::Any, k::Any)
    if time < lag
        return 0.0
    else        
        return perturb * exp(-k * ((time % t_p)))
...
# e.g. pulse_expd_periodic(0, time, 1.5, 84, 0.2) vs. pulse_expd_periodic(0, time, 1.2, 84, 0.2)


#/ periodic pulses (with a time lag in the beginning) + exponentially decreasing with time, with perturbation parameter (>1, treated as the inversed decay parameter as initial strength of pulse), pulse period t_p (hour) and the peak constant k (fold of μ_i) defined
# (the bigger the perturb, the slower the decrease)
...
# e.g. pulse_expd_periodic2(0, times[i], 4, 84, 1.5) vs. pulse_expd_periodic2(0, times[i], 2, 84, 1.5)

## Code to visualize the perturbation impact (e.g. from pulse_expd_periodic2)
function perturbation_visualizaiton(link_function, param_values)
    time = 0:500
    y1 = Array{Float64}(undef, 0)
    y2 = Array{Float64}(undef, 0)
    y3 = Array{Float64}(undef, 0)
    y4 = Array{Float64}(undef, 0)
    y5 = Array{Float64}(undef, 0)
    
    for i in 1:length(time)
     _y1 = link_function(0, time[i], param_values[1], 84, 1.5)
     _y2 = link_function(0, time[i], param_values[2], 84, 1.5)
     _y3 = link_function(0, time[i], param_values[3], 84, 1.5)
     _y4 = link_function(0, time[i], param_values[4], 84, 1.5)
     _y5 = link_function(0, time[i], param_values[5], 84, 1.5)
    ...
    plot(time, [y1,y2,y3,y4,y5])

end

# (e.g. from pulse_nd_single)
function perturbation_visualizaiton2(link_function, param_values)
    time = 0:500
    y1 = Array{Float64}(undef, 0)
    y2 = Array{Float64}(undef, 0)
    y3 = Array{Float64}(undef, 0)
    ...
    for i in 1:length(time)
     _y1 = link_function(time[i], param_values[1], 150)
     _y2 = link_function(time[i], param_values[2], 150)
     _y3 = link_function(time[i], param_values[3], 150)
     _y4 = link_function(time[i], param_values[4], 150)
     _y5 = link_function(time[i], param_values[5], 150)
...
# e.g. 
# params = [1,2,3,4,5]
# perturbation_visualizaiton(pulse_expd_periodic2, params)
# perturbation_visualizaiton2(pulse_nd_single, params)

## Function to update the death rates with time-dependent pertubatoin impact
...
# in this old version, the density-dependent terms in the equations of per capita competition rate and per capita infection rate are taken care of later while calculating the total rates (see gillepie.jl; search for "Compute rates")
function get_interaction_rates(states, params)
    #/ Instantiate per capita competition rate and infection rate
    competition_rates = Array{Array{Float64, 1}, 1}(undef, 0)
    infection_rates = Array{Array{Float64, 1}, 1}(undef, 0)
    ...
    for (focal_substrain, focal_plasmid_profile) in zip(states.subpopulations, states.plasmid_profiles)
        #/ Allocate arrays
        ...
        for (other_substrain, other_plasmid_profile) in zip(states.subpopulations, states.plasmid_profiles)
            competitor_id = other_substrain[begin] #/ get the competitior's (or the non-focal substrain's) strain id
            # strain_abundance = sum(states.abundances[states.subpopulations .== competitor_id]) #/ get the other substrain's strain abundance
            #/ Compute the per-capita competition rate divided by carrying capacity
            
            # Option 1.1: condider both intraspecific and interspecific competition, and K_i
            # η = (isempty(_plasmids)) ?
            ...
            # _A = η * params.A[strain_id,competitor_id] / params.carrying_capacity[strain_id]

            # Option 1.2: condider both intraspecific and interspecific competition and K (e.g. = K_i)
            η = (isempty(_plasmids)) ?
            params.growth_rate[strain_id] :
            params.growth_rate[strain_id] * reduce((x, y) -> x * (1.0 - y), params.plasmid_cost[_plasmids], init=1.0) # taking the sequential product of (1-cost) from each plasmid           
            ...
            # Option 2: condider only interspecific competition and K_i
            # strain_abundance = sum(states.abundances[states.subpopulations .== strain_id]) #/ get the focal substrain's strain abundance
            # η = (isempty(_plasmids)) ?
            # params.growth_rate[strain_id] * (1- strain_abundance / params.carrying_capacity[strain_id]) :
            # params.growth_rate[strain_id] * (1- strain_abundance / params.carrying_capacity[strain_id]) * reduce((x, y) -> x * (1.0 - y), params.plasmid_cost[_plasmids], init=1.0) # taking the sequential product of (1-cost) from each plasmid

            # _A = (strain_id == competitor_id) ?
            # 0.0 : η * params.A[strain_id,competitor_id] / params.carrying_capacity[strain_id]

            # Option 3: condider only interspecific competition and K = sum of K_i
            # K = sum(params.carrying_capacity) #/ get community carrying capacity
            # N = sum(states.abundances) #/ get the community abundance
            # η = (isempty(_plasmids)) ?
            # params.growth_rate[strain_id] * (1- N / K) :
            # params.growth_rate[strain_id] * (1- N / K) * reduce((x, y) -> x * (1.0 - y), params.plasmid_cost[_plasmids], init=1.0) # taking the sequential product of (1-cost) from each plasmid

            # _A = (strain_id == competitor_id) ?
            # 0.0 : η * params.A[strain_id,competitor_id] / K
            
            push!(_competition_rates, _A)

            #/ old version: If the focal subpouplation has any plasmid that the other doesn't have, infection rate (γ_{i,p}) = strain-specific values, otherwise infeciton rate = 0
            if any(focal_plasmid_profile .> 0) && any(focal_plasmid_profile .> other_plasmid_profile)
                #/ Get the non-focal substrain's abundance
                #substrain_indice = [i for i in 1:length(states.subpopulations) if states.subpopulations[i] == competitor_id && states.plasmid_profiles[i] == other_plasmid_profile]
                #substrain_abundance = states.abundances[substrain_indice[begin]]
                #/ Push infection rate 
                push!(_infection_rates, params.infection_rate[focal_substrain]) #(infection coefficient)
                #push!(_infection_rates, params.infection_rate[focal_substrain] * substrain_abundance) # (infection coefficient * abundance)
            else
                #/ Otherwise, no infect, so push 0
        ...
        push!(infection_rates, _infection_rates) # this belongs to the old version
    end
 ...
 # in this new version, the density-dependent term of the new equation of per capita infeciton rate (i.e. only sum up the abundances of the recipients) is considered in the funciton below
function get_interaction_rates2(states, params)
    ...
    for  (focal_substrain_idx,(focal_substrain, focal_plasmid_profile)) in enumerate(zip(states.subpopulations, states.plasmid_profiles)) 
        ...
        #/ new version: calculate per-capita infection rate, i.e γ_{i,p} * sum (recipient abundance)
        if any(focal_plasmid_profile .> 0)
            # note: If the focal subpouplation is a donor (i.e. has at least 1 plasmid), infection rate (γ_{i,p}) = strain-specific values, otherwise γ_{i,p} = 0
            # note: focal_sbustrain_idx is the subpopulation index in the system, where findfirst(x -> x == focal_substrain_idx, states.donors) is the subpopulation index in the donor list
...
# Function for propensity tensor = n * n * n where n = number of all possilbe plasmid profiles (not used).
# function get_propensities(params; power::Float64 = 2.0)
#     #/ To compute the propensity tensor P, we need a list of all different plasmid
...
#     Γ = zeros(Float64, np, np, np)
#     #/ Loop through all possible plasmid profiles and infection pathways
#     for (donor, focal_plasmid_profile) in enumerate(all_profiles) #/ go through all potential donors' plasmid profiles 
#         for (recipient, other_plasmid_profile) in enumerate(all_profiles) # go through all potential recipients' plasmid profiles
...
#             #/ If there are transferrable plasmids, check all combinations
...
#                     transconjugant = findfirst(==(new_profile), all_profiles)
#                     #/ Define propensity tensor element
#                     #!Note: We assume here that infecting 'p' plasmids scales as 1/2^(p-1)
#                     Γ[transconjugant,donor,recipient] = 1.0 / 2^(sum(infection_profile)-1) # (note: 2 can be replaced by "power"?)
#                 end
#                 #/ Normalize propensity tensor so that it can be used as a weight vector
#                 #!Note: We do not necessarily need to do this, as weight vectors can be
#                 #       arbitrary numbers, it is only their relative value that is important.
#                 #       However when it is normalized, the propensity tensor gains biological
#                 #       interpretation as Γ[i,j,k] = P[i,j,k] * H[j,k]  (note: i = transconjugant's plasmid profile in matrix P; j & k = donor's & recipient's plasmid profiles/strains in matrix P/H)  
#                 #       (where Γ infection rate and H the horizontal gene transfer matrix)
#                 #!Note: We do the normalization here to avoid division by 0
#                 Γ[:,donor, recipient] ./= sum(Γ[:, donor, recipient])
#             end
#         end
#     end
#     #/ Return propensity tensor as a statically-sized array (using StaticArrays)
#     PStaticArray = SArray{Tuple{np, np, np}}(Γ)
#     return PStaticArray
# end

# Function for propensity tensor = nm * nm * nm where n = number of all possilbe plasmid profiles and m = number of all possible bacterial strains.
# function get_propensities2(params; power::Float64 = 2.0)
...
#     np = length(all_profiles) #/ number of all possible plasmid profiles in the system = 2^(n_pstrains)
#     nb = params.n_bstrains #/ number of all possible strain profiles (i.e. strain IDs) in the system
#     rep_strains = repeat([1, 2, 3], inner=np)
#     rep_profiles = repeat(all_profiles, outer=nb)
    
#     #/ Allocate propensity tensor 
#     Γ = zeros(Float64, np * nb, np * nb, np * nb)
...
#                     #/ Filter element with matrix H (for H size > 1 x 1, as 1 x 1 H will always have a single element = 1)
#                     if size(params.H, 1) > 1
#                         if params.H[other_strain, focal_strain] == 0
#                             Γ[transconjugant,donor,recipient] = 0
#                         end
#                     end    

#                     _all_plasmids = findall(new_profile .> 0)
#                     for plasmid in _all_plasmids
#                         #/ Filter element with matrix I (for I has > 1 elements, as a 1 x 1 I will always have a single element = 1)
#                         if length(params.I) > 1
#                             if  params.I[other_strain,plasmid] == 0 
#                                 Γ[transconjugant,donor,recipient] = 0
#                             end
#                         end    
#                         #/ Filter element with matrix P (except for P_ii)
#                         for other_plasmid in _all_plasmids 
#                             if params.P[other_plasmid,plasmid] == 0 && other_plasmid != plasmid
#                                 Γ[transconjugant,donor,recipient] = 0     
#                             end    
#                         end
#                     end
...
#                 #!Note: We do the normalization here to avoid division by 0
#                 Γ[:,donor, recipient] ./= sum(Γ[:, donor, recipient])
#                 # Replace NaNs (due to sum(P[:, donor, recipient] = 0) with 0
#                 Γ = replace(Γ, NaN => 0) 
#             end
#         end
#     end
#     #/ Return propensity tensor as a statically-sized array (using StaticArrays)
#     PStaticArray = SArray{Tuple{np * nb, np * nb, np * nb}}(Γ)
#     return PStaticArray
# end


# Upgraded function for propensity tensor = nm * nm * nm where n = number of all possilbe plasmid profiles and m = number of all possible bacterial strains.
...
# Upgraded & simplified (consider only uni-plasmid transmission) function for propensity tensor = nm * nm * nm where n = number of all possilbe plasmid profiles and m = number of all possible bacterial strains.
# Note: this function used sparse array to save memory and increase iteration efficiency. 
function get_propensities_simple(params)
    #/ To compute the propensity tensor P, we need a list of all different strain ID & plasmid
    start_time = now()
    combs = Iterators.product([[0,1] for i in 1:params.n_pstrains]...) #/ create tuple (i.e. immutable collection of elements) of vectors[0,1] repeated by n_pstrains
    all_profiles = collect.(vec([comb for comb in combs])) #/ create all possible plasmid profiles in the system
