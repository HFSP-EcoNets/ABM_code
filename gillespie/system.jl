...
function get_background_rates(
	...
	for (strain_id, plasmid_profile) in zip(states.subpopulations, states.plasmid_profiles)
        ...
        if perturbation[strain_id] == 0
            ...
            μ = (isempty(_plasmids)) ? 
            death_rate[strain_id] * (1 + pulse_expd_periodic2(100.0, time, perturbation[strain_id], 100, 9.0)) :
            death_rate[strain_id] * (1 + pulse_expd_periodic2(100.0, time, perturbation[strain_id], 100, 9.0) * (1 - maximum(plasmid_resistance[_plasmids])))    
        end
