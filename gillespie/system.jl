...
#= Peturbation mode: periodic, exponentially decreasing =#
#/ Start module
module System
...
function get_background_rates(
	...
	for (strain_id, plasmid_profile) in zip(states.subpopulations, states.plasmid_profiles)
        ...
        #~ Growth rate (not considering intra-specific competition)
        _plasmids = findall(plasmid_profile .> 0) # list plasmids hosted by the focal subpopulation
...
# The 3-dimension propensity tensor has the size of = nm * nm * nm where n = number of all possilbe plasmid profiles and m = number of all possible bacterial strains.
